...
public class ImagePreviewAdapter extends PagerAdapter implements PhotoViewAttacher.OnPhotoTapListener {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        //FiveGridView.getImageLoader().onDisplayImage(view.getContext(), imageView, info.bigImageUrl);

        pb.setVisibility(View.VISIBLE);
        Glide.with(context).load(info.bigImageUrl)//
                .placeholder(R.drawable.ic_camera)//
                .error(R.drawable.photo_fail)//
...
//        Bitmap cacheImage = FiveGridView.getImageLoader().getCacheImage(imageInfo.bigImageUrl);
        //如果大图的缓存不存在,在获取小图的缓存
//        if (cacheImage == null) cacheImage = FiveGridView.getImageLoader().getCacheImage(imageInfo.thumbnailUrl);
